...
class PatchSampleFtoBCAN(nn.Module):
    def __init__(self, nce_feature_dim_list, gpu_ids=[0], use_mlp=False, init_type='normal', init_gain=0.02, nc=256):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleFtoBCAN, self).__init__()
        self.l2norm = Normalize(2)
        ...
        self.init_gain = init_gain
        self.gpu_ids = gpu_ids
        for mlp_id, feat_dim in enumerate(nce_feature_dim_list):
            input_nc = feat_dim
            mlp = MLPNet(input_size=input_nc, hidden_size=self.nc, num_classes=self.nc).cuda()
