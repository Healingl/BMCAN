...
def Hausdorff_distance95(ref, seg):
    """
    This functions calculates the average symmetric distance and the
    hausdorff distance 95 between a segmentation and a reference image
    :return: hausdorff distance and average symmetric distance
    """
    ref_border_dist, seg_border_dist = border_distance(ref, seg)
    hausdorff_distance_95 = np.max([np.percentile(ref_border_dist[ref + seg > 0], q=95),
                                    np.percentile(seg_border_dist[ref + seg > 0], q=95)])
