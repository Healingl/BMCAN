...
def validate_by_real_mr_patient(seg_model,seg_decoder, epoch):
    """
...
if __name__ == "__main__":
    model_config_path = './config/model_config/BMCAN_CT2MR.yaml'
    ...
    patch_sample_f_s = PatchSampleFtoBCAN(nce_feature_dim_list=nce_feature_dim_list, gpu_ids=gpu_list,use_mlp=True, init_type='kaiming', init_gain=0.02, nc=256)
    patch_sample_f_t = PatchSampleFtoBCAN(nce_feature_dim_list=nce_feature_dim_list, gpu_ids=gpu_list, use_mlp=True, init_type='kaiming', init_gain=0.02, nc=256)
    ...
    for i_iter in tqdm(range(model_yaml_config['resume'], num_steps + 1), total=num_steps - model_yaml_config['resume']):
        ...
        # ct2mr
        source_data, source_label, target_data, target_label = train_ct_input.cuda(
            non_blocking=True), train_ct_label.cuda(non_blocking=True), train_mr_input.cuda(
            non_blocking=True), train_mr_label.cuda(non_blocking=True)
