...
if __name__ == "__main__":
    ...
    for i_iter in tqdm(range(model_yaml_config['resume'], num_steps + 1), total=num_steps - model_yaml_config['resume']):
        ...
        if i_iter % model_yaml_config['iter_val'] == 0 and i_iter != 0:
            ...
            mean_dice_value, mean_assd_value, eval_dsc_dict, eval_assd_dict = validate_by_real_ct_patient(seg_model=seg_shared,
        ...
        # ct2mr
        source_data, source_label, target_data, _ = train_mr_input.cuda(non_blocking=True), train_mr_label.cuda(non_blocking=True), train_ct_input.cuda(non_blocking=True), train_ct_label.cuda(non_blocking=True)
        ...
        if model_yaml_config['lambda_adv_i'] > 0:
            ...
            if model_yaml_config['lambda_s2t_seg'] > 0 and i_iter > model_yaml_config['start_s2t_seg']:
                ...
                loss_CC = Cor_CoeLoss(rec_s2t.detach(), sdatav.detach()) + Cor_CoeLoss(rec_t2s.detach(),
                                                                                       tdatav.detach())
